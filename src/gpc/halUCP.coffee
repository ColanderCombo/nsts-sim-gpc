...
SVC_ERROR_MESSAGES = {
    ...
    22: "CHARACTER TO INTEGER CONVERSION"
...
export class HalUCP
  ...
  # Return false to let the CPU perform the SVC's standard PSW swap.
  handleSVC: (ea, r1) ->
    ...
    # A program without I/O has no IOINIT but can still halt through
    # %SVCI(0) when no SVC new PSW is installed.
    if svcCode == 0x0015 and (@active or @cpu.ram.get32(0x5c) == 0)
    ...
    # Remaining UCP services require IOINIT; otherwise hardware handles them.
    ...
    if svcCode == 0x0014
      ...
      if @errorCallback
        ...
        process.stderr.write msg + "\n"
      # Dispatch through ON ERROR when a matching handler exists.
  ...
  _extractNextField: (iocode) ->
    ...
    # Skip leading whitespace (not commas or semicolons, which are significant)
  ...
  # at `caller_stack_base + {18,19}`, immediately after the 18-hw
