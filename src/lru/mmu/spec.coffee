# mmu -- a Mass Memory Unit and the tape volumes it serves
#
...
export SPEC =
  id:      'mmu'
  title:   'Mass Memory Unit'
  summary: 'Space Shuttle Mass Memory Unit -- device model and tape tools'
  usage: [
    'Examples:'
    '  mmu run --unit 1 --volume tape.mmv'
    '  mmu create tape.mmv'
    '  mmu put tape.mmv 0/0/0/0 somefile.bin'
    '  mmu get tape.mmv 0/0/0/0 --blocks 2 -o out.bin'
    '  mmu ls tape.mmv'
    '  mmu dump tape.mmv 0/0/0/0'
    '  mmu watch MM1'
  ]
  tables: ['create', 'put', 'get', 'ls', 'dump']

  run:
    summary: 'run a mass memory unit on its bus and answer a GPC'
    options: [
      ['--unit <n>', 'mass memory unit 1 or 2 (bus MM1 / MM2)', '1']
      ['--bus <name>', 'override the bus this unit hangs on']
      ['--volume <file>', 'tape volume to serve (default: a blank tape)']
      ['--write-protect', 'refuse every write']
      ['--fault-on-blank', 'report a data dropout when a block was never written']
      ['--reply-delay <ms>', 'delay before answering a command', '0']
      ['--block-delay <ms>', 'delay between the blocks of a transfer']
      ['--save-on-exit', 'write the volume back when the process is stopped']
      ['quiet', 'do not trace commands']
    ]
    build: (o) ->
      vol = if o.volume then openVolume(o.volume) else new Volume()
